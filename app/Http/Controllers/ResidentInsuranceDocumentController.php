...
class ResidentInsuranceDocumentController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            // بناء الاستعلام
            $query = ResidentInsuranceDocument::with(['passengers', 'branchAgent', 'user']);
            ...
            Log::error('Error in ResidentInsuranceDocumentController@index: ' . $e->getMessage());
    ...
    /**
     * Print resident insurance document.
     */
    public function print(string $id)
    {
        try {
            $document = ResidentInsuranceDocument::with(['passengers', 'branchAgent'])->findOrFail($id);
            ...
            return view('resident-insurance-documents.print', compact('document', 'printData'));
        } catch (\Exception $e) {
            Log::error('Error in ResidentInsuranceDocumentController@print: ' . $e->getMessage());
    ...
    public function store(Request $request)
    {
        try {
            $validated = $request->validate([
                ...
                'geographic_area' => 'nullable|in:داخل ليبيا (للأفراد),داخل ليبيا (للعائلات)',
                'residence_type' => 'nullable|in:تأشيرة إقامة Residence Visa,تأشيرة عمل Work Visa',
                ...
                'passengers.*.whatsapp_number' => 'nullable|string|max:255',
                ...
                'passengers.*.occupation' => 'nullable|string|max:255',
            ]);
        ...
        try {
            ...
            $lastDocument = ResidentInsuranceDocument::where('insurance_number', 'like', 'MLEPT%')
                ->orderBy('id', 'desc')
                ->first();
            if ($lastDocument && preg_match('/MLEPT(\d+)/', $lastDocument->insurance_number, $matches)) {
                $nextNumber = (int)$matches[1] + 1;
            } else {
                $nextNumber = 1;
            }
            do {
                $insuranceNumber = 'MLEPT' . str_pad($nextNumber, 5, '0', STR_PAD_LEFT);
                $nextNumber++;
            } while (ResidentInsuranceDocument::where('insurance_number', $insuranceNumber)->exists());
            ...
            // إنشاء الوثيقة
            $document = ResidentInsuranceDocument::create([
                ...
                'tax' => $validated['tax'] ?? 2.500,
                'stamp' => $validated['stamp'] ?? 0.500,
                'issue_fees' => $validated['issue_fees'] ?? 10.000,
                'supervision_fees' => $validated['supervision_fees'] ?? 1.050,
            ...
            foreach ($validated['passengers'] as $passengerData) {
                ResidentInsurancePassenger::create([
                    ...
                    'occupation' => $passengerData['occupation'] ?? null,
                ]);
            }

            return response()->json($document->load('passengers'), 201);
        } catch (\Exception $e) {
            Log::error('Error in ResidentInsuranceDocumentController@store: ' . $e->getMessage());
            return response()->json([
    ...
    {
        try {
            $document = ResidentInsuranceDocument::with('passengers')->findOrFail($id);
            ...
            Log::error('Error in ResidentInsuranceDocumentController@show: ' . $e->getMessage());
            return response()->json([
    ...
    public function update(Request $request, string $id)
    {
        ...
        try {
            ...
            Log::error('Error in ResidentInsuranceDocumentController@update: ' . $e->getMessage());
            return response()->json([
