...
return new class extends Migration
{
    ...
    {
        if (!Schema::hasTable('personal_accident_insurance_documents')) {
            Schema::create('personal_accident_insurance_documents', function (Blueprint $table) {
                $table->id();
                // بيانات التأمين
                $table->string('insurance_number'); // MLPSA00001
                ...
                $table->string('id_proof')->nullable(); // إثبات شخصي
                ...
                $table->string('claim_authorized_name')->nullable(); // اسم الموكل للمطالبات
                // القيمة المالية
                $table->decimal('premium', 10, 3)->default(200.000); // القسط المقرر
                ...
                $table->unique('insurance_number', 'pai_insurance_number_unique');
            });
        }
    }
    ...
    {
        Schema::dropIfExists('personal_accident_insurance_documents');
    }
};
